package com.blackink.app.presentation.screens.setup
...
fun Setup2Screen(vm: SetupViewModel, onContinue: () -> Unit, onBack: () -> Unit) {
    ...
    SetupShell(step = 2, title = "What should we call your business?", subtitle = "This name organizes your inventory, reports, and partner settlements.", onContinue = onContinue, onBack = onBack) {
        ...
        FlipCard {
            FieldLabel("Preview")
            Text(
                state.businessName.ifBlank { "Your business name" },
                style = FlipTheme.typography.headingL,
                color = if (state.businessName.isBlank()) FlipTheme.colors.textWeakest else FlipTheme.colors.textDefault,
            )
        }
    }
}
